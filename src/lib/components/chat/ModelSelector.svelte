<script lang="ts">
	...
	$: if (
		...
		!$models.some((m: any) => m.name === selectedModels[0])
	) {
		...
		if (thirdParty) {
			...
			const smallest = ollamaModels.reduce(
				(min: any, m: any) => ((m.size || 0) < (min.size || 0) ? m : min),
				ollamaModels[0]
			);
...
{#if compact}
	<select
		class="ui-chip min-w-0 max-w-[150px] cursor-pointer appearance-none pr-7 sm:max-w-[250px]"
	...
	<div class="flex flex-col my-2">
		{#each selectedModels as selectedModel, selectedModelIdx}
			<div class="flex items-center gap-2">
				<select
					id="models"
					class="ui-select block w-full text-lg font-semibold placeholder-gray-400"
