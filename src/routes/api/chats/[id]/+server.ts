	...
	try {
		...
		await pool.execute(
			...
			[
				...
				JSON.stringify(
					updated.options !== undefined ? updated.options : safeJsonParse(chat.options)
				),
				JSON.stringify(
					updated.messages !== undefined ? updated.messages : safeJsonParse(chat.messages)
				),
				JSON.stringify(
					updated.history !== undefined ? updated.history : safeJsonParse(chat.history)
				),
...
export const DELETE: RequestHandler = async ({ params, request }) => {
	try {
		...
		await pool.execute("DELETE FROM chats WHERE id = ? AND username = ?", [
			params.id,
			auth.username
		]);
		return json({ success: true });
