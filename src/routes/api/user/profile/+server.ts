...
export async function PUT({ request }) {
	try {
		...
		if (newPassword !== undefined) {
			...
			if (!(await verifyPassword(String(oldPassword), pwdRows[0].password))) {
		...
		if (
			...
			newEmail !== undefined ||
			avatar !== undefined ||
			systemAvatar !== undefined
		) {
			if (newUsername !== undefined && newUsername !== username) {
				const [existing] = await pool.execute<User[]>("SELECT id FROM users WHERE username = ?", [
					newUsername
				]);
			...
			if (updates.length > 0) {
				...
				await pool.execute(`UPDATE users SET ${updates.join(", ")} WHERE username = ?`, params);
		...
		return json({ error: "未提供任何更新内容" }, { status: 400 });
	} catch (error) {
